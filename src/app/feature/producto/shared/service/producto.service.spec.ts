...
describe('ProductoService', () => {
  ...
  const apiEndpointProductos = `${environment.endpoint}/users`;
  ...
  it('deberia listar productos', () => {
    const dummyProductos = {
      data: [
        { id: '1', email: 'Producto 1' }, { id: '2', email: 'Producto 2' }
      ]
    };
    service.consultar().subscribe(productos => {
      expect(productos.length).toBe(2);
      expect(productos).toEqual(dummyProductos.data);
    });
    const req = httpMock.expectOne(apiEndpointProductoConsulta);
    expect(req.request.method).toBe('GET');
    req.flush(dummyProductos.data);
  });

  it('deberia crear un producto', () => {
    const dummyProducto = { id: '1', email: 'Producto 1' };
